...
if [ "$#" -ne 5 ]; then
    ...
    echo "Usage: ./bin_seg_shim.sh <dicoms_path> <subject_name> <diameter> < blur_width > <verification>"
    echo "Example: ./bin_seg_shim.sh /path/to/dicoms subject_name 25 9 1"
    exit 1
fi
...
if [ $VERIFICATION == 1 ] && [ -f "$FIELDMAP_PATH" ]; then
...
if [ ! -d $OPTI_OUTPUT_DIR ]; then
    mkdir $OPTI_OUTPUT_DIR
fi

# Run the shim for the segmentation
OUTPUT_DIR="${OPTI_OUTPUT_DIR}/dynamic_shim_${SUBJECT_NAME}_segmentation"
echo -e "\nShimming the fieldmap for the segmentation..."
st_b0shim dynamic \
...
# Run the shim for the binary mask
OUTPUT_DIR="${OPTI_OUTPUT_DIR}/dynamic_shim_${SUBJECT_NAME}_binary_mask"
echo -e "\nShimming the fieldmap for the binary mask..."
st_b0shim dynamic \
    --coil $COIL_PATH $COIL_CONFIG_PATH \
    --fmap $FIELDMAP_PATH \
    --anat $EPI_PATH \
    --mask $FNAME_BIN_MASK_SCT \
    --mask-dilation-kernel-size 3 \
    --optimizer-criteria 'rmse' \
    --optimizer-method "least_squares" \
    --slices "auto" \
