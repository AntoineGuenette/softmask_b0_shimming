...
if [ $VERIFICATION == 1 ] && [ -f "$FNAME_SEGMENTATION" ]; then
    ...
    python run_inference_single_subject.py \
        -i "${MPRAGE_PATH}" \
        -path-model /Users/antoineguenette/Desktop/Scolaire/NeuroPoly/Stage_E25/Experiences/spinalcordtoolbox/data/deepseg_models/model_seg_sc_contrast_agnostic_nnunet/nnUNetTrainer__nnUNetPlans__3d_fullres \
